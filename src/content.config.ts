...
const notices = defineCollection({
  ...
  schema: z.object({
    ...
    // 2026-08-19 작업지시서 — 별도 대표 사진 필드(image/imageAlt) 폐지. 본문(body)에
    // 삽입한 첫 번째 마크다운 이미지를 카드 목록 썸네일로 재사용한다(src/lib/noticeImage.ts).
    // 본문에 이미지가 없으면 로고를 기본 이미지로 표시. 상세페이지에는 별도 배너 없이
    // 본문 이미지가 흐름 안에서 그대로 보인다.
