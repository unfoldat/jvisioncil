...
const STATIC_ENTRIES = [
  ...
  { page: '소식', href: '/소식/', title: '소식', snippet: '좋은비전의 활동 소식을 전합니다.', keys: '소식 공지 공지사항 게시판 새소식 활동 알림 뉴스' },
  ...
  { page: '센터소개', href: '/기관소개/#about-way', title: '오시는 길', snippet: `${orgInfo.data.address}. 지하철·버스 안내.`, keys: '오시는길 위치 주소 약도 지도 응암 은평 교통 방문 찾아오는길' },
...
export async function GET() {
  ...
  const noticeEntries = notices.map((notice) => ({
    page: '소식',
    href: noticeHref(notice).href,
    title: notice.data.title,
    snippet: '소식',
    keys: notice.data.title,
  }));
