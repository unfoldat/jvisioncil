...
## 강사 프로필 심플(bio) vs 상세(rich) 판정 기준 (2026-08-20)

`intro/education/career/certifications` 중 하나라도 있으면 상세(rich)
레이아웃, 없고 `bio`(단순 리스트)만 있으면 심플 레이아웃으로 렌더한다
(`isSimple` 불리언, `InstructorProfile.astro`). 메인 강사(김기현)만
상세 스키마를 쓰고, CMS로 신규 추가하는 강사는 전부 심플 — CMS 폼에도
rich 필드는 의도적으로 노출하지 않는다("B안" 확정, 심플 카드는
`.tutor-profile-simple`로 폭 50%로 축소).

---

## 소식 카드 sr-only "위쪽 이미지" 추가→제거 결정 (2026-08-26)

포스터형(정보가 이미 본문 텍스트에 중복 존재하는 이미지) vs 사진형
(이미지 자체가 유일한 정보원인 이미지)을 구분해, 사진형에만 Tab 전용
sr-only 안내("위쪽 이미지")를 붙이기로 했었으나, 실제 소식 썸네일을
검토한 결과 대부분 포스터형이라 판단 기준 자체가 실질적으로 불필요 —
`NoticeList.astro`에서 sr-only 안내를 완전히 제거. 판단 기준은
`content-structure-a11y` 스킬 7절에 재사용 가능하도록 남겨둠.

---

## 푸터 전화번호 안내 문구 제거 원칙 (2026-08-26)

`tel:` 링크는 스크린리더가 이미 "link"로 네이티브 낭독하므로
"선택하면 전화가 연결됩니다" 같은 동작 설명을 aria-label에 중복
추가하지 않는다(사실관계도 부정확 — 실제로는 다이얼러만 채워질 뿐
자동 통화 연결이 아님). 다만 식별 라벨("전문상담 대표전화")은
시각적 라벨과 프로그램적으로 연결돼 있지 않으므로 유지.

---

## CSS Grid `min-width:auto` 오버플로우 반복 패턴 (2026-08-26)

`display:grid`에 `grid-template-columns`를 명시하지 않으면, 그리드
아이템의 기본 `min-width:auto` 때문에 가장 넓은 자식(긴 미분리
텍스트 등)이 암시적 트랙 폭을 그 min-content로 밀어버려, 같은 트랙을
공유하는 다른(짧은) 카드까지 전부 컨테이너 밖으로 넘친다. 이 세션에서
`.tutor-list` 등 최소 2회 동일 패턴 발생 — 해법은 항상
`grid-template-columns: minmax(0, 1fr)` 명시.

---

## 카카오맵 버튼 44px 규칙 충돌 + 새창→같은창 원칙 (2026-08-26)

사이트 전역 접근성 규칙 `button{min-height:44px}`가 Kakao roughmap이
런타임에 주입하는 줌 버튼(고정 크기 104px 컨테이너에 맞춰진 위젯)까지
덮어써 버튼이 컨테이너를 넘치며 Tab 포커스 시 잘려 보이는 버그로
이어짐 — `:global(.root_daum_roughmap button){min-height:0;padding:0}`로
그 위젯 안에서만 전역 규칙을 무효화(전역 규칙 자체는 유지). 같은
섹션의 "길찾기" 링크도 새 탭이 아닌 같은 창에서 열리도록 `target`
속성을 제거(프로젝트 원칙 35번 — 새 창을 열 특별한 이유가 없으면
같은 창에서 연다).
